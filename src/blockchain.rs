use crate::zhtp::zk_consensus::ZkNetworkMetrics;
use chrono::Utc;
...
impl Transaction {
    ...
    pub fn sign(&mut self, private_key: &str) {
        ...
        // Extract the actual signature part (after the colon)
        if let Some(signature_part) = self.signature.split(':').nth(1) {
            // Verify that the signature was created with the claimed public key
            let hash = self.calculate_hash();
            let expected_signature = format!("{}:{}", public_key, hash);
...
impl Blockchain {    pub fn new(base_reward: f64) -> Self {
    ...
    pub async fn add_transaction(&self, transaction: Transaction) -> bool {
        ...
        if transaction.nonce != expected_nonce {
            println!("Invalid nonce: expected {}, got {}", expected_nonce, transaction.nonce);
            return false;
        }
